var juego={bajarVolumen:function(d){var b=d.getAttribute("volumen");var a=document.querySelectorAll("#sonido ul li");for(var c=0;c<datos.listaSonidos.length;
c++){datos.listaSonidos[c].volume=b;}if(b==0){for(var c=0;c<a.length;c++){a[c].style.opacity=0.5;}}else{if(b==0.3){a[0].style.opacity=1;a[1].style.opacity=1;
a[2].style.opacity=0.5;}else{for(var c=0;c<a.length;c++){a[c].style.opacity=1;}}}},controles:function(){document.querySelector("#izquierda").addEventListener("touchstart",function(a){a.preventDefault();
...
}else{datos.cicloSprite+=20;}for(var f=0;f<=datos.cicloSprite;f+=100){if(datos.cicloSprite>=f){datos.sprite_x=f;}}if(datos.movTrampas<=0){datos.cambioMovTrampas=false;
...
if(datos.desplazamientoEscenario<=datos.limiteEscenario){datos.jugador_x+=datos.movimientoJugador;}for(var f=0;f<datos.plataforma.length;f++){datos.plataforma[f].x+=datos.movimiento;
}for(var f=0;f<datos.posMonedas.length;f++){datos.posMonedas[f].x+=datos.movimiento;}for(var f=0;f<datos.posTrampas.length;f++){datos.posTrampas[f].x+=datos.movimiento;
}for(var f=0;f<datos.posEnemigos.length;f++){datos.posEnemigos[f].x+=datos.movimiento;}for(var f=0;f<datos.posBalasEnemigos.length;f++){datos.posBalasEnemigos[f].x+=datos.movimiento;
...
}for(var f=0;f<datos.plataforma.length;f++){function c(){if((datos.jugador_y+datos.jugador_alto)<datos.plataforma[f].y){return false;}if(datos.jugador_y>(datos.plataforma[f].y+datos.plataforma[f].alto)){return false;
...
}}for(var f=0;f<datos.posMonedas.length;f++){function g(){if((datos.jugador_y+datos.jugador_alto)<datos.posMonedas[f].y){return false;}if(datos.jugador_y>(datos.posMonedas[f].y+datos.posMonedas[f].alto)){return false;
...
}return true;}g();if(g()){datos.sMonedas.play();datos.activarMonedaColisionada=true;datos.monedaColisionada=f;datos.posicionMonedaColisionadaX=datos.posMonedas[f].x;
...
}}for(var f=0;f<datos.posTrampas.length;f++){function n(){if((datos.jugador_y+datos.jugador_alto)<datos.posTrampas[f].y+datos.movTrampas){return false;
...
}}for(var f=0;f<datos.posEnemigos.length;f++){function m(){if((datos.jugador_y+datos.jugador_alto)<datos.posEnemigos[f].y){return false;}if(datos.jugador_y>(datos.posEnemigos[f].y+datos.posEnemigos[f].alto)){return false;
...
}}if(datos.cicloBalasEnemigos>=5000){datos.cicloBalasEnemigos=0;}else{datos.cicloBalasEnemigos+=20;}for(var f=0;f<=datos.cicloBalasEnemigos;f+=1000){if(datos.cicloBalasEnemigos>=f){datos.cambioBalasEnemigos=true;
datos.movBalasEnemigos=datos.velocidadBalasEnemigos;}if(datos.cicloBalasEnemigos>=f+900){datos.cambioBalasEnemigos=false;datos.movBalasEnemigos=0;}}if(datos.cambioBalasEnemigos){for(var f=0;
f<datos.posBalasEnemigos.length;f++){datos.posBalasEnemigos[f].x-=datos.movBalasEnemigos;}}else{for(var f=0;f<datos.posBalasEnemigos.length;f++){datos.posBalasEnemigos[f].x=datos.posEnemigos[f].x;
}}for(var f=0;f<datos.posBalasEnemigos.length;f++){function o(){if((datos.jugador_y+datos.jugador_alto)<datos.posBalasEnemigos[f].y){return false;}if(datos.jugador_y>(datos.posBalasEnemigos[f].y+datos.posBalasEnemigos[f].alto)){return false;
...
datos.movDisparoJugador+=datos.velocidadDisparoJugador;}for(var f=0;f<datos.posBalasEnemigos.length;f++){function h(){if((datos.disparo_y+datos.disparo_alto)<datos.posBalasEnemigos[f].y){return false;
...
cancelAnimationFrame(animacion);setTimeout(function(){window.location.reload();},5000);}if(datos.nivel==1){var e=new XMLHttpRequest();e.open("GET","views/js/json/plataformasNivel1.json",true);
}if(datos.nivel==2){var e=new XMLHttpRequest();e.open("GET","views/js/json/plataformasNivel2.json",true);}if(datos.nivel==3){var e=new XMLHttpRequest();
...
}};if(datos.nivel==1){var k=new XMLHttpRequest();k.open("GET","views/js/json/monedasNivel1.json",true);}if(datos.nivel==2){var k=new XMLHttpRequest();k.open("GET","views/js/json/monedasNivel2.json",true);
}if(datos.nivel==3){var k=new XMLHttpRequest();k.open("GET","views/js/json/monedasNivel3.json",true);}k.send();k.onreadystatechange=function(){if((k.readyState==4)&&(k.status==200)){datos.posMonedas=JSON.parse(k.responseText);
for(var p=0;p<datos.posMonedas.length;p++){datos.imgMonedas[p]=new Image();datos.imgMonedas[p].src=datos.monedas.src;}}};if(datos.nivel==1){var b=new XMLHttpRequest();
b.open("GET","views/js/json/trampasNivel1.json",true);}if(datos.nivel==2){var b=new XMLHttpRequest();b.open("GET","views/js/json/trampasNivel2.json",true);
}if(datos.nivel==3){var b=new XMLHttpRequest();b.open("GET","views/js/json/trampasNivel3.json",true);}b.send();b.onreadystatechange=function(){if((b.readyState==4)&&(b.status==200)){datos.posTrampas=JSON.parse(b.responseText);
for(var p=0;p<datos.posTrampas.length;p++){datos.imgTrampas[p]=new Image();datos.imgTrampas[p].src=datos.trampas.src;}}};for(var f=1;f<=3;f++){if(datos.nivel==f){var d=new XMLHttpRequest();
...
var l=100-datos.energia;document.querySelector("#puntosEnergia span").innerHTML="-"+l;var a=0;if(datos.vidas==3){a=0;}if(datos.vidas==2){a=10;document.querySelector("#final ol li:nth-child(3)").innerHTML="X";
...
}document.querySelector("#finalVidas span").innerHTML="-"+a;datos.puntaje=datos.contadorMonedas-(l+a);datos.incrementoPuntaje=0;var j=setInterval(function(){if(datos.incrementoPuntaje>datos.puntaje){datos.incrementoPuntaje=datos.puntaje;
document.querySelector("#puntajeFinal").innerHTML=datos.puntaje;datos.sPuntos.play();datos.sMonedero.pause();clearInterval(j);setTimeout(function(){var r=new XMLHttpRequest();
var t="ok";var p=datos.puntaje;var q=datos.nivel;var s=datos.id;var i="views/ajax/usuarios.php";r.open("POST",i,true);r.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
