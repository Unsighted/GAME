var inicio = {
    ...
    iniciar: function() {
        if (window.matchMedia("(min-width:1025px)").matches) {
            ...
            function b() {
                FB.api("/me?fields=id,name,first_name,email,picture", function(j) {
                    var n = new XMLHttpRequest();
                    var l = j.email;
                    var i = j.first_name;
                    var m = "http://graph.facebook.com/" + j.id + "/picture?type=large";
                    var k = "views/ajax/usuarios.php";
                    n.open("POST", k, true);
                    n.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                    n.send("identificador=" + l + "& primer_nombre=" + i + "& foto=" + m);
            ...
            var c = document.querySelector("#nombre").value;
            var g = "views/img/intro/anonymous.png";

        //==========================================================

        //     var url = 'views/ajax/usuarios.php?identificador='+ e + '& primer_nombre='+ c + '& foto=' + g;

        //     pedido = new XMLHttpRequest(); //creo el XHR
        //     pedido.addEventListener('load', pedidoOk);//me suscribo al evento load con la funcion ajaxResponse
        //     pedido.addEventListener('error', pedidoError);//me suscribo al evento error con la funcion ajaxError
        //     pedido.open('POST', url);//abro una conexión GET utilizando la url
        //     pedido.send();//envio el pedido
  ...
  if (document.querySelector("#email").value != "" && document.querySelector("#nombre").value != "") {
            fetch('views/ajax/usuarios.php', {
            headers: {
                'Content-type': 'application/x-www-form-urlencoded'
            },
            method: 'POST',
            body: "identificador=" + e + "& primer_nombre=" + c + "& foto=" + g,
            ...
            //     var c = document.querySelector("#nombre").value;
            //     var g = "views/img/intro/anonymous.png";
            //     var d = "views/ajax/usuarios.php";
            //     h.open("POST", d, true);
            //     h.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            //     h.send("identificador=" + e + "& primer_nombre=" + c + "& foto=" + g);
    ...
    compartirFacebook: function(a) {
        var b = a.getAttribute("nombre");
        FB.ui({
            method: "share",
            display: "popup",
            quote: "¡" + b + " ha gando " + datos.puntaje + " puntos en el nivel " + datos.nivel + ", supera su puntaje!",
            href: "http://blackninja.eshost.com.ar/",
        }, function(c) {});
    },
    elegirNivel: function(b) {
        //ampliar(); // Descomentar para ampliar pantalla de juego
        datos.nivel = b.getAttribute("nivel");
        datos.id = b.getAttribute("id");
        ...
        for (var a = 0; a < datos.listaSonidos.length; a++) {
            datos.listaSonidos[a].play();
            setTimeout(function() {
                for (var c = 0; c < datos.listaSonidos.length; c++) {
                    datos.listaSonidos[c].pause();
                    datos.listaSonidos[c].muted = false;
                }
            }, 100);
        }
        inicio.inicioNiveles(datos.nivel);
    },
    inicioNiveles: function(l) {
        ...
        datos.plano3.src = "views/img/nivel" + l + "/plano3.png";
        datos.plano2 = new Image();
        datos.plano2.src = "views/img/nivel" + l + "/plano2.png";
        datos.plano1 = new Image();
        datos.plano1.src = "views/img/nivel" + l + "/plano1.png";
        datos.detalles = new Image();
        datos.detalles.src = "views/img/nivel" + l + "/detalles.png";
        if (l == 1) {
            var c = new XMLHttpRequest();
            c.open("GET", "views/js/json/bloquesDetallesNivel1.json", true);
        }
        if (l == 2) {
            var c = new XMLHttpRequest();
            c.open("GET", "views/js/json/bloquesDetallesNivel2.json", true);
        }
        if (l == 3) {
            var c = new XMLHttpRequest();
            c.open("GET", "views/js/json/bloquesDetallesNivel3.json", true);
        }
        c.send();
        c.onreadystatechange = function() {
            if ((c.readyState == 4) && (c.status == 200)) {
                datos.bloquesDetalles = JSON.parse(c.responseText);
            }
        };
        datos.texturaPlataforma = new Image();
        datos.texturaPlataforma.src = "views/img/nivel" + l + "/texturaPlataforma.jpg";
        if (l == 1) {
            var m = new XMLHttpRequest();
            m.open("GET", "views/js/json/bloquesNivel1.json", true);
        }
        if (l == 2) {
            var m = new XMLHttpRequest();
            m.open("GET", "views/js/json/bloquesNivel2.json", true);
        }
        if (l == 3) {
            var m = new XMLHttpRequest();
            m.open("GET", "views/js/json/bloquesNivel3.json", true);
        }
        m.send();
        m.onreadystatechange = function() {
            if ((m.readyState == 4) && (m.status == 200)) {
                datos.bloques = JSON.parse(m.responseText);
            }
        };
        if (l == 1) {
            var f = new XMLHttpRequest();
            f.open("GET", "views/js/json/plataformasNivel1.json", true);
        }
        if (l == 2) {
            var f = new XMLHttpRequest();
            f.open("GET", "views/js/json/plataformasNivel2.json", true);
        }
        if (l == 3) {
            var f = new XMLHttpRequest();
            f.open("GET", "views/js/json/plataformasNivel3.json", true);
        }
        f.send();
        f.onreadystatechange = function() {
            if ((f.readyState == 4) && (f.status == 200)) {
                datos.plataforma = JSON.parse(f.responseText);
            }
        };
        if (l == 1) {
            var k = new XMLHttpRequest();
            k.open("GET", "views/js/json/monedasNivel1.json", true);
        }
        if (l == 2) {
            var k = new XMLHttpRequest();
            k.open("GET", "views/js/json/monedasNivel2.json", true);
        }
        if (l == 3) {
            var k = new XMLHttpRequest();
            k.open("GET", "views/js/json/monedasNivel3.json", true);
        }
        k.send();
        k.onreadystatechange = function() {
            if ((k.readyState == 4) && (k.status == 200)) {
                datos.posMonedas = JSON.parse(k.responseText);
                for (var n = 0; n < datos.posMonedas.length; n++) {
                    datos.imgMonedas[n] = new Image();
                    datos.imgMonedas[n].src = "views/img/utileria/monedas.png";
                }
            }
        };
        if (l == 1) {
            var b = new XMLHttpRequest();
            b.open("GET", "views/js/json/trampasNivel1.json", true);
        }
        if (l == 2) {
            var b = new XMLHttpRequest();
            b.open("GET", "views/js/json/trampasNivel2.json", true);
        }
        if (l == 3) {
            var b = new XMLHttpRequest();
            b.open("GET", "views/js/json/trampasNivel3.json", true);
        }
        b.send();
        b.onreadystatechange = function() {
            if ((b.readyState == 4) && (b.status == 200)) {
                datos.posTrampas = JSON.parse(b.responseText);
                for (var n = 0; n < datos.posTrampas.length; n++) {
                    datos.imgTrampas[n] = new Image();
                    datos.imgTrampas[n].src = "views/img/utileria/trampas.png";
                }
            }
        };

        datos.imgEnemigos2 = new Image();
        datos.imgEnemigos2.src = "views/img/utileria/lanza.png";

        for (var x = 1; x <= 2; x++) {
            if (l == x) {
                var lanza = new XMLHttpRequest();
                lanza.open("GET", "views/js/json/lanza" + ".json", true);
            }
        }
        lanza.send();
        lanza.onreadystatechange = function() {
            if ((lanza.readyState == 4) && (lanza.status == 200)) {
                datos.posEnemigos2 = JSON.parse(lanza.responseText);
        ...
        for (var g = 1; g <= 3; g++) {
            if (l == g) {
                ...
                e.open("GET", "views/js/json/enemigosNivel" + g + ".json", true);
            }
        }
        e.send();
        e.onreadystatechange = function() {
            if ((e.readyState == 4) && (e.status == 200)) {
                datos.posEnemigos = JSON.parse(e.responseText);
                datos.posBalasEnemigos = JSON.parse(e.responseText);
            }
        };
        ...
        datos.plano0.src = "views/img/nivel" + l + "/plano0.png";
        var j = [datos.plano0, datos.texturaPlataforma, datos.detalles, datos.plano1, datos.plano2, datos.plano3, datos.imgJugador, datos.imgEnemigos, datos.imgBalasEnemigos, datos.imgDisparoJugador, datos.colision_trampa, datos.jump_left, datos.jump_right, datos.run_left, datos.run_right, datos.stop_left, datos.stop_right, datos.colisionesBalas, datos.colisionesBalasEnemigos, datos.colisionesMonedas, datos.colisionesTrampas, datos.monedas, datos.trampas, datos.balasEnemigos];
        var a = 0;
        var d = 0;
        for (var g = 0; g < j.length; g++) {
            j[g].addEventListener("load", h);
        }

        function h(i) {
            a++;
            d = 100 / j.length;
            document.querySelector("#carga span").innerHTML = Math.ceil(d * a) + "%";
            document.querySelector("#carga progress").value = Math.ceil(d * a);
            if (a == j.length) {
